...
class RobotPost(object):
    ...
    MAX_VJ = 300.0  # system maximum joint speed deg/s
    STR_VJ = str(60/180*3.14159)       # set default joints speed
    STR_ACC_JOINT = str(100/180*3.14159)       # set default joints ACC
    # Rounding value (from 0 to 4) (in RoboDK, set to 100 mm rounding for PL=4
    STR_PL = '0'
    ...
    # Count the number of instructions (limited by MAX_LINES_X_PROG)
    LINE_COUNT = 0
    ...
    # Original name of the current program (example: ProgA)
    PROG_NAME = 'unknown'
    # Auto generated name (different from PROG_NAME if we have more than 1 page per program. Example: ProgA2)
    ...
    def ProgStart(self, progname, new_page=False):
        ...
        if new_page:
            ...
            if self.nPages == 0:
                ...
                # add the first program in the list to be genrated as a subprogram call
    ...
    def progsave(self, folder, progname, ask_user=False, show_result=False):
        if ask_user or not DirExists(folder):
            folder = getSaveFolder(
                folder, 'Select a directory to save your program')
            if folder is None:
        ...
        # tell RoboDK the path of the saved file
        print('SAVED: %s\n' % filesave)
        ...
        if nfiles >= 1:
            ...
            for i in range(len(self.PROG_LIST)):
                ...
                if show_result and i > 3:
                    show_result = False
                self.progsave(
                    folder_user, self.PROG_NAMES[i], False, show_result)
    ...
    def ProgSendRobot(self, robot_ip="192.168.1.200", remote_path="/rbctrl/", robot_user="root", robot_pwd="elite2014"):
        ...
        try:
            ...
            ShowMessage(
                "pysftp module not found ,please use \"pip install pysftp\" to install pysftp module", "module not found")
        ...
        print(
            f"robot_ip:{robot_ip},robot_user:{robot_user},robot_pwd:{robot_pwd}")
        for file in self.PROG_FILES:
            # ShowMessage(file)
            # ShowMessage(remote_path + get_file_name(file))

            sftp_download(robot_ip, robot_user, robot_pwd, file,
                          remote_path + get_file_name(file))
    ...
    def setFrame(self, pose, frame_id, frame_name):
        ...
        self.User1 = pose
        User1tmp = pose_2_xyzrpw(pose)

        #str_out_user1 = '    <Comment comment="add User1[%.1f,%.1f,%.1f,%.1f,%.1f,%.1f] in Config-Frame" typeName="Comment"/>\n' % (User1tmp[0],User1tmp[1],User1tmp[2],User1tmp[3],User1tmp[4],User1tmp[5])
        str_out_user1 = '    <Comment comment="在 配置-坐标系 添加坐标系 User1[%.1f,%.1f,%.1f,%.1f,%.1f,%.1f]" typeName="Comment"/>' % (User1tmp[0],User1tmp[1],User1tmp[2],User1tmp[3],User1tmp[4],User1tmp[5])
      
        self.addline(str_out_user1)
    ...
    def setTool(self, pose, tool_id=None, tool_name=None):
        ...
        #str_out_tcp = '    <Comment comment="Modify TCP data [%.1f,%.1f,%.1f,%.1f,%.1f,%.1f] in Config-TCP" typeName="Comment"/>\n' % (tool1_tmp[0],tool1_tmp[1],tool1_tmp[2],tool1_tmp[3],tool1_tmp[4],tool1_tmp[5])
        str_out_tcp = '    <Comment comment="在 配置 - TCP中添加[%.1f,%.1f,%.1f,%.1f,%.1f,%.1f]并激活" typeName="Comment"/>' % (tool1_tmp[0],tool1_tmp[1],tool1_tmp[2],tool1_tmp[3],tool1_tmp[4],tool1_tmp[5])
    ...
    def MoveJ(self, pose, joints, conf_RLF=None):
        ...
        if pose is not None:
            ...
            # out_wp_name = self.wp_name+str(self.P_COUNT)
            # out_joint = ''
            # for i in range(0, 5):
            ...
            # str_wp_node = '      <WaypointNode name=\"'+out_wp_name+'\" positionNodeType="FIXED_POSITION" kinematicFlag="-1" customTransitionRadius="true" advancedPositionOptionType="INHERITED_PARAMETERS">\n        <transitionRadius>\n          <valueInSi>' + \
            #     self.STR_PL+'</valueInSi>\n        </transitionRadius>\n        <internalPosition>\n          <jointPositions joints='
            # str_wp_node += '\"'+out_joint+'\" />\n'
            # str_wp_node += '          <toolPose ' + \
            #     out_pose+'/>\n        </internalPosition>\n'
            # str_wp_node += '      <jointSpeed>\n        <valueInSi>'+self.STR_VJ + \
            #     '</valueInSi>\n      </jointSpeed>\n     </WaypointNode>'
            # self.insertline(str_wp_node)
    ...
    def MoveC(self, pose1, joints1, pose2, joints2, conf_RLF_1=None, conf_RLF_2=None):
        ...
        if pose1 is not None:
            ...
            if self.bUseFrame:
                self.addline('movec(pose_trans(user1,%s),pose_trans(user1,%s),a=1.05,%s,%s,mode = 1)' % (
                    target_pose1, target_pose2, self.STR_V, self.STR_PL))
            else:
                self.addline('movec(%s,%s,a=1.05,%s,%s,mode = 1)' % (
                    target_pose1, target_pose2, self.STR_V, self.STR_PL))
    ...
    def waitDI(self, io_var, io_value, timeout_ms=-1):
        ...
        if type(io_value) != str:
            if io_value > 10:
                io_value = 10
        if timeout_ms > 0:
            self.addline('WAIT %s=%s T=%s' % (io_var, str(
                io_value), str(round(timeout_ms/1000, 3))))
        else:
