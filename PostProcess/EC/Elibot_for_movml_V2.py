...
class RobotPost(object):
    ...
    STR_PL = 'PL=0'             # Rounding value (from 0 to 4) (in RoboDK, set to 100 mm rounding for PL=4
    ...
    def ProgFinish(self, progname, new_page = False):
        ...
        header_ins = ''
        header_ins += '//trajfile' + '\n'
        header_ins += 'version2.15' + '\n'
        header_ins += 'interval [XXXX]' + 'ms\n'
        header_ins += 'refJointPos [' + self.refJointPos+']\n'
        pid = self.P_COUNT
        header_ins += 'length [%i]' % pid + '\n'
        header_ins += 'postype [pose]' + '\n'
        header_ins += 'outputNumber [2]'
    ...
    def progsave(self, folder, progname, ask_user = False, show_result = False):
        ...
        with open(filesave, "w", encoding="utf-8") as fid:
            ...
            # fid.write(str(len(self.PROG)))
            i=0
            for line in self.PROG:
                if line.find("'") == -1 and i!=len(self.PROG):
                    fid.write(line)
                    fid.write('\n')
                i=i+1
    ...
    def setFrame(self, pose, frame_id, frame_name):
        ...
        for i,k in enumerate(pose_2_xyzrpw(pose)):
            ...
            if i < 5 :
                user_pose = user_pose + ','
        self.addline("refFrame [%s]" % user_pose)
    ...
    def MoveJ(self, pose, joints, conf_RLF=None):
        ...
        if pose is not None:
            ...
            self.addline('MOVJ P%03i %s %s' % (pid, self.STR_VJ, self.STR_PL))
    ...
    def MoveL(self, pose, joints, conf_RLF=None):
        ...
        if pose is not None:
            ...
            self.P_COUNT = self.P_COUNT + 1
            if self.bFirst:
                joints_ex = [str(x) for x in joints]
                self.refJointPos = ','.join(joints_ex)
                self.bFirst = False
    ...
    def setZoneData(self, zone_mm):
        ...
        if zone_mm < 0:
            ...
            self.STR_PL = 'PL=%i' % round(min(zone_mm, 7))
