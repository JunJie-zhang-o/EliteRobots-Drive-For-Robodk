...
class RobotPost(object):
    ...
    STR_V = 'V=100.0MM/S'         # set default cartesian speed
    ...
    def MoveJ(self, pose, joints, conf_RLF=None):
        ...
        if pose is not None:
            ...
            #joints_ex = ["%.3f"%x for x in joints]
       
            joints_str = '['
            for i in range(0,6):
                joints_str = joints_str + str(round(joints[i],3))
                if i<5:
                    joints_str = joints_str+','
            joints_str = joints_str+']'
            
            #self.addline('SETJOINT P%03i ' % pid + ','.join(joints_ex))
            #MOVEJ VJ=50% CR=0.0MM ConstP=[-18.091,-92.057,101.056,-114.736,91.536,47.457]
            self.addline('MOVEJ %s %s ACC=20 DEC=20 ConstP=%s' %(self.STR_VJ, self.STR_PL,joints_str))
    ...
    def MoveL(self, pose, joints, conf_RLF=None):
        ...
        if pose is not None:
            ...
            self.addline('MOVEL %s %s ACC=20 DEC=20 TOOL#(1) USER#(1) ConstV=[%s]' %(self.STR_V, self.STR_PL,target_pose))
 
            # MOVEL V=200.0MM/S CR=0.0MM ACC=20 DEC=20 TOOL#(0) USER#(0) ConstV=[377.3906552,-11.7858445,309.8881949,-2.9724018,0.2190102,-2.6924639]
    ...
    def MoveC(self, pose1, joints1, pose2, joints2, conf_RLF_1=None, conf_RLF_2=None):
        ...
        if pose1 is not None:
            ...
            self.addline('MOVEC %s %s ACC=20 DEC=20 TOOL#(1) USER#(1) ConstV=[%s] ConstV=[%s]' %(self.STR_V, self.STR_PL,target_pose1,target_pose2))
